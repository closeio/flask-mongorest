...
class MongoRestTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.post_1 = {
            ...
            # user_lists
            "sections": [
                {"text": "this is the first section of the first post.", "lang": "en"},
                {"text": "this is the second section of the first post.", "lang": "de"},
                {"text": "this is the third section of the first post.", "lang": "fr"},
            ],
            "content": {"text": "this is the content for my first post.", "lang": "cn"},
            "is_published": True,
        }

        self.post_2 = {"title": "Second post", "is_published": False}
    ...
    def test_model_validation_unicode(self):
        ...
        resp = self.app.post(
            "/user/",
            data=json.dumps({"email": "test@example.com", "datetime": "invalid"}),
        )
        response_error(resp)
        errors = resp_json(resp)
        self.assertEqual(
    ...
    def test_resource_fields(self):
        resp = self.app.post(
            "/testfields/",
            data=json.dumps(
                {"name": "thename", "other": "othervalue", "upper_name": "INVALID"}
            ),
        )
        response_success(resp)
        obj = resp_json(resp)
    ...
    def test_post(self):
        ...
        resp = self.app.put(
            "/posts/", data=json.dumps({"description": "Other description"})
        )
        response_success(resp)
        data = resp_json(resp)
        ...
        resp = self.app.put(
            "/posts/", data=json.dumps({"description": "X" * 121})  # too long
        )
        response_error(resp)
        data = resp_json(resp)
    ...
    def test_pagination(self):
        ...
        self.assertEqual(
            data["error"],
            "The limit you set is larger than the maximum limit for this resource (max_limit = 100).",
        )
        ...
        self.assertEqual(
            data["error"],
            "The limit you set is larger than the maximum limit for this resource (max_limit = 10).",
        )
        ...
        self.assertEqual(
            data["error"],
            "The limit you set is larger than the maximum limit for this resource (max_limit = 250).",
        )
    ...
    def test_chunked_request(self):
        resp = self.app.post(
            "/a/",
            data=json.dumps({"txt": "test"}),
            headers={"Transfer-Encoding": "chunked"},
        )
        response_error(resp, code=400)
        self.assertEqual(
            resp_json(resp), {"error": "Chunked Transfer-Encoding is not supported."}
        )
    ...
    def test_view_methods(self):
        ...
        resp = self.app.post("/test_view_method/", data=json.dumps({"txt": "doc2"}))
        response_success(resp)
        ...
        resp = self.app.put("/test_view_method/", data=json.dumps({"txt": "doc"}))
        response_success(resp)
    ...
    def test_methods_success(self):
        ...
        resp = self.app.post("/create_only/", data=json.dumps({"txt": "created"}))
        response_success(resp)
    ...
    def test_fetch_method_permissions(self):
        ...
        # create
        resp = self.app.post("/fetch_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=404)
        ...
        # bulk put
        resp = self.app.put("/fetch_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=404)
    ...
    def test_list_method_permissions(self):
        ...
        # create
        resp = self.app.post("/list_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=405)
        ...
        # bulk put
        resp = self.app.put("/list_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=405)
    ...
    def test_create_method_permissions(self):
        ...
        # create
        resp = self.app.post("/create_only/", data=json.dumps({"txt": "works"}))
        response_success(resp)
        ...
        # bulk put
        resp = self.app.put("/create_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=405)
    ...
    def test_update_method_permissions(self):
        ...
        # create
        resp = self.app.post("/update_only/", data=json.dumps({"txt": "works"}))
        response_error(resp, code=404)
        ...
        # bulk put
        resp = self.app.put("/update_only/", data=json.dumps({"txt": "doesnt work"}))
        response_error(resp, code=404)
    ...
    def test_bulk_update_method_permissions(self):
        ...
        # bulk put
        resp = self.app.put("/bulk_update_only/", data=json.dumps({"txt": "works"}))
        response_success(resp)
        ...
        # create
        resp = self.app.post("/bulk_update_only/", data=json.dumps({"txt": "works"}))
        response_error(resp, code=405)
    ...
    def test_delete_method_permissions(self):
        ...
        # bulk put
        resp = self.app.put("/delete_only/", data=json.dumps({"txt": "works"}))
        response_error(resp, code=404)
        ...
        # create
        resp = self.app.post("/delete_only/", data=json.dumps({"txt": "works"}))
        response_error(resp, code=404)
    ...
    def test_bulk_update_limit(self):
        ...
        resp = self.app.put("/posts/", data=json.dumps({"title": "Title"}))
        response_error(resp, code=400)
        self.assertEqual(
            resp_json(resp),
            {"errors": ["It's not allowed to update more than 10 objects at once"]},
        )
        self.assertEqual(
            11, example.documents.Post.objects.filter(title__ne="Title").count()
        )
...
class MongoRestSchemaTestCase(unittest.TestCase):
    ...
    def test_person(self):
        resp = self.app.post(
            "/person/",
            data=json.dumps(
                {"name": "John", "languages": [{"name": "English"}, {"name": "German"}]}
            ),
        )
        response_success(resp)
    ...
    def test_datetime(self):
        resp = self.app.post(
            "/datetime/", data=json.dumps({"datetime": "2010-01-01T00:00:00"})
        )
        response_success(resp)
    ...
    def test_receive_bad_json(self):
        ...
        response_error(resp, code=400)
        self.assertEqual(
            resp_json(resp), {"error": "The request contains invalid JSON."}
        )
    ...
    def test_send_bad_json(self):
        ...
        doc = example.DictDoc.objects.create(
            dict={"NaN": float("NaN"), "inf": float("inf"), "-inf": float("-inf")}
        )
...
class InternalTestCase(unittest.TestCase):
    ...
    def test_serialize_mongoengine_validation_error(self):
        ...
        error = ValidationError(errors={"a": ValidationError("Invalid value")})
        result = serialize_mongoengine_validation_error(error)
        self.assertEqual(result, {"field-errors": {"a": "Invalid value"}})

        error = ValidationError("Invalid value")
        result = serialize_mongoengine_validation_error(error)
        self.assertEqual(result, {"error": "Invalid value"})

        error = ValidationError(errors={"a": "Invalid value"})
        result = serialize_mongoengine_validation_error(error)
        self.assertEqual(result, {"field-errors": {"a": "Invalid value"}})
